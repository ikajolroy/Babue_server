...
exports.addStudy =async (req, res) => {
    ...
    try {
        ...
        if(!subcategory||!name){
            ...
            if(image) {fs.unlinkSync(imageSplit)}
            if(audio){fs.unlinkSync(audioSplit)}
        ...
        const {name,subcategory,poem} = req.body;
            ...
            if(findSubcategory.layout ==="poem"){
                ...
                if(image) {
                    ...
                    if(prevImage){fs.unlinkSync(prevImage)}
                if(audio&&prevAudio){fs.unlinkSync(prevAudio)}
            }else{
...
exports.deleteStudy = async (req, res) => {
    try {
        ...
        if(prevImage){fs.unlinkSync(prevImage)}
        if(prevAudio){fs.unlinkSync(prevAudio)}
